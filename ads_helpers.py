...
RE_DATA_PATTERN = r'Timestamp:\s+(\d+\.\d+)\s+ID:\s+(\w+)\s+(\w+)\s+DLC:\s+(\d+)\s+((?:\w{2}\s+)+)'
RE_NULL_DATA_PATTERN = r'Timestamp:\s+(\d+\.\d+)\s+ID:\s+(\w+)\s+(\w+)\s+DLC:\s+(\d+)'
...
def txt_to_df(dataset_dict:dict, data_dir:str, dataset:str) -> pd.DataFrame:
    ...
    with open(f'{data_dir}/{dataset_dict[dataset]}', 'r') as f:
        for line in f:
            re_match = re.match(RE_DATA_PATTERN, line)
            if re_match:
                ...
                re_match = re.match(RE_NULL_DATA_PATTERN, line)
                if re_match:
